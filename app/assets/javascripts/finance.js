...
$(function() {
		...
		if (Array.isArray(data)) {
			$.each(data, function(i, folder) {
				html.push("<li class='folder' data-id='" + folder.id + "'>"
						...
						+ "<span class='folderTotal'>" + "  &ndash; $"
						+ folder.total + "</span>" + "</a>" + "</li>");
			});
		} else {
			html.push("<li class='folder' data-id='" + data.id + "'>"
					+ "<a class ='folder-name' >" + data.name
					+ "<span class='folderTotal'>" + "  &ndash; $" + data.total
					+ "</span>" + "</a>" + "</li>");
		}
	...
	var deleteFolder = function(event) {
		...
		jsRoutes.controllers.FinanceFolderController.deleteFolder(folderId)
				.ajax({
					success : function(data) {
	...
	var generateTransFormNode = function(transaction) {
		...
		if (transaction) {
			...
			node = "<tr data-id='"
					+ transaction.id
					+ "' class='updateRow' style='display: none'>"
					...
					+ "</div></td>"
					// Add custom key to be the same as form value so that
					// update row and view row stay in the same place
					+ "<td class='transDate' sorttable_customkey='"
					+ (date.getUTCMonth() + 1)
					+ '/'
					+ date.getUTCDate()
					+ '/'
					+ date.getUTCFullYear()
					+ "'>"
					+ "<input type='text' size='15' class='form-control datepicker ' value='"
					+ (date.getUTCMonth() + 1)
					+ '/'
					+ date.getUTCDate()
					+ '/'
					+ date.getUTCFullYear()
					+ "'>"
					+ "</td> <td class='transDescript' sorttable_customkey='"
					+ transaction.shortDescription
					+ "'>"
					+ "<input type='text' class='form-control ' value='"
					+ transaction.shortDescription
					+ "'>"
					+ "</td><td class='transAmount' sorttable_customkey='"
	...
	var addTransaction = function(row) {
		jsRoutes.controllers.TransactionController
				...
				.ajax(
						{
							data : {
								amount : parseFloat($('.transAmount input', row)
										.val()),
	...
	var deleteTransCallback = function(event) {
		...
		$('#transaction-table .actionBox input[type=checkbox]:checked').each(
				function(i, checkbox) {
					transactionIds.push($(checkbox).closest('tr').attr(
							'data-id'));
				});
		if (transactionIds.length > 0) {
			var confirmed = confirm("Delete " + transactionIds.length
					+ " transaction(s)?");

			// if user confirms deletion for all selected items, call ajax
			// function for
							...
							.ajax(
									{
										success : function(data) {
											var removedIndexes = [];
											$(
													"#transaction-table tr[data-id='"
															+ data.id + "']")
													.remove();
											// update selectedFolderObj to
											// reflect deleted transaction
											selectedFolderObj.total -= data.amount;
											$
													.each(
															selectedFolderObj.transactions,
															function(i,
																	transaction) {
											...
											$.each(removedIndexes, function(i,
													index) {
												selectedFolderObj.transactions
														.splice(index, 1);
											});
											updateSelectedFolderTotal();
											// updates table for sorting, but
											// doesnt resort
											updateTableSort(false);
										}
									});
	...
	var updateTransCallback = function(event) {
		var transactionIds = [];
		$('#transaction-table input[type=checkbox]:checked').each(
				function(i, checkbox) {
					transactionIds.push($(checkbox).closest('tr').attr(
							'data-id'));
				});
		...
		if (transactionIds.length > 0) {
			$.each(transactionIds, function(i, transaction) {
				$(".viewRow[data-id='" + transaction + "']").css('display',
						'none');
	...
	var updateTransaction = function(updatedRow) {
		...
		jsRoutes.controllers.TransactionController
				...
				.ajax(
						{
							data : {
								amount : parseFloat($('.transAmount input',
										updatedRow).val()),
								shortDescription : $('.transDescript input',
										updatedRow).val(),
								creationDate : $('.transDate input', updatedRow)
										.val()
							},
							success : function(data) {
								// updates viewrow to what was entered in
								// updated row
								...
								$('.transDate ', viewRow)[0].innerHTML = (date
										.getUTCMonth() + 1)
										+ '/'
										+ date.getUTCDate()
										+ '/'
								...
								$('.transAmount', updatedRow).attr(
										'sorttable_customkey',
										data.amount.toFixed(2));
								$('.transDescript', updatedRow).attr(
										'sorttable_customkey',
										data.shortDescription);
								$('.transDate', updatedRow).attr(
										'sorttable_customkey',
										(date.getUTCMonth() + 1) + '/'
												+ date.getUTCDate() + '/'
												+ date.getUTCFullYear());
	...
	var removeFormView = function(event, row) {
		...
		if (row.hasClass('updateRow')) {
			...
			$(".transDescript input", row).val(
					$(".transDescript", viewRow).html());
	...
	var updateSelectedFolderTotal = function() {
		$(".selected .folderTotal")[0].innerHTML = "  &ndash; $"
				+ selectedFolderObj.total.toFixed(2);
	};
	...
	var updateTableSort = function(resort) {
		...
		// tells table sorter that table has been updated and resorts it if
		// resrot == true
